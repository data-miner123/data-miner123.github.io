...
const state={site:null,user:null,reports:[],literature:[],publications:[],publicationPending:false,users:[],adminMembers:[],filter:'all',query:'',literatureQuery:'',detailRequest:0,literatureRequest:0,currentLiterature:null};
...
function renderAdmin(){
  ...
  $('#pending-count').textContent=`${state.users.filter(u=>u.status==='pending').length} 个待审核账号`;$('#admin-report-count').textContent=`${state.reports.length} 份研究报告`;$('#admin-literature-count').textContent=`${state.literature.length} 篇文献`;$('#admin-publication-count').textContent=`${state.publications.length} 项公开成果`;
  $('#publication-publish-status').textContent=state.publicationPending?'有尚未发布的成果改动。确认内容后请点击“发布到 GitHub Pages”。':'公开成果已与本地 Git 状态同步。';$('#publication-publish-status').className=`publish-status ${state.publicationPending?'pending':'synced'}`;$('#publish-publications').disabled=!state.publicationPending;
  $('#admin-publications').innerHTML=state.publications.map(publicationAdminRow).join('')||'<p class="empty-state">还没有公开成果。点击“新增成果”开始添加。</p>';
